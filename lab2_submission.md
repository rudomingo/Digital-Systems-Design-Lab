...
```scala
Our original implementation of GEMM, shown above, ran for 21677 cycles when passed in the arguments "32 32 32". After looking at the controller tree page, we noticed that 21673 of the 21677 cycles were performed in the outer loop over k which had a parallelization factor of 1. Since the outer loop does not carry any loop dependencies between each iteration, we knew that we could increase the parallelization factor of this loop without affecting the outcome of the calculation so we increased outerPar to equal 2. With the new configuration of outerPar = midPar = innerPar = 2, the GEMM ran for only 12696 cycles with the same arguments passed as our original test. At this point, however, we then tried to synthesize our design in order to obtain the resource utilization but it led to errors during the make process. After this we were no longer able to synthesize any future configurations nor were we even able to make any of the vcs simulations for some reason. So this was the only results we were able to obtain.
```
